...
export default function Login() {
  ...
  /* ─────────────────────────────────────────────────────────
     MOBILE LAYOUT — pure white, no-scroll, native-app feel
  ───────────────────────────────────────────────────────── */
  if (isMobile) {
    return (
      <div style={{
        ...
        {/* Subtle aurora glow — decorative only, zero layout impact */}
        <div style={{ position: 'absolute', top: -120, left: '50%', transform: 'translateX(-50%)', width: 420, height: 320, borderRadius: '50%', background: 'radial-gradient(ellipse,rgba(108,99,255,.13) 0%,transparent 70%)', pointerEvents: 'none' }} />
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '52px 24px 32px', position: 'relative' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            <h1 style={{ fontSize: 28, fontWeight: 900, color: '#0f1117', margin: '0 0 6px', letterSpacing: '-.4px' }}>
              Welcome back
            </h1>
            <p style={{ fontSize: 14, color: '#9ca3af', margin: 0 }}>Sign in to your account to continue</p>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '20px 0' }}>
            ...
            <span style={{ fontSize: 12, color: '#d1d5db', fontWeight: 500, whiteSpace: 'nowrap' }}>New to PriceTracker?</span>
            <div style={{ flex: 1, height: 1, background: '#f0f0f4' }} />
          </div>
          ...
          <Link to="/register" style={{ textDecoration: 'none' }}>
            <div style={{
              ...
              <UserPlus size={17} color="#6c63ff" /> Create a free account
            </div>
          </Link>

          {/* Trust row */}
          <div style={{ display: 'flex', justifyContent: 'space-around', marginTop: 24, padding: '18px 0', borderTop: '1px solid #f3f4f6', borderBottom: '1px solid #f3f4f6' }}>
            {[
      ...
      <div style={{
        ...
        {DOTS.map(([l, t], i) => (
          <div key={i} style={{
            ...
            background: 'rgba(255,255,255,.12)', left: `${l}%`, top: `${t}%`,
          }} />
        ))}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '32px 0', position: 'relative' }}>
          <h1 style={{ fontSize: 44, fontWeight: 900, color: '#fff', margin: '0 0 14px', lineHeight: 1.1, letterSpacing: '-1.5px' }}>
            Smarter shopping<br />
            <span style={{ background: 'linear-gradient(90deg,#818cf8,#a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>starts here</span>
          </h1>
          <p style={{ color: '#7b82a8', fontSize: 15, lineHeight: 1.7, margin: '0 0 36px', maxWidth: 320 }}>Track prices, get alerts, and never overpay on Amazon again.</p>
          ...
          <div style={{ position: 'absolute', right: -16, top: '44%', transform: 'translateY(-50%)' }}>
            <div style={{ width: 192, background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 20, padding: '16px 16px 12px', backdropFilter: 'blur(12px)', boxShadow: '0 20px 60px rgba(0,0,0,.4)' }}>
              ...
              <svg width="160" height="52" viewBox="0 0 160 52" style={{ display: 'block' }}>
                ...
                <polyline points="0,38 26,30 52,36 80,20 108,24 136,10 160,4" fill="none" stroke="#818cf8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            ...
            <div style={{ position: 'absolute', top: -16, right: 18, width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#6c63ff,#a78bfa)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 20px rgba(108,99,255,.6)' }}>
              <Bell size={15} color="#fff" fill="#fff" />
              <span style={{ position: 'absolute', top: -3, right: -3, width: 14, height: 14, borderRadius: '50%', background: '#ef4444', border: '2px solid #0f1035', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, color: '#fff', fontWeight: 900 }}>3</span>
            </div>
          </div>
        </div>
      ...
      <div style={{ width: 500, flexShrink: 0, display: 'flex', flexDirection: 'column', background: '#fff' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 60px' }}>
          <div style={{ marginBottom: 32 }}>
            <h2 style={{ fontSize: 32, fontWeight: 900, color: '#0f1117', margin: 0, letterSpacing: '-.5px' }}>Welcome back</h2>
            <p style={{ fontSize: 14, color: '#9ca3af', margin: '8px 0 0' }}>Sign in to your account to continue</p>
          </div>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>Password</label>
                <Link to="/forgot-password" style={{ fontSize: 13, color: '#6c63ff', textDecoration: 'none', fontWeight: 600 }}>Forgot password?</Link>
              </div>
              <div style={{ position: 'relative' }}>
                ...
                <input type={showPass ? 'text' : 'password'} required value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} onFocus={() => setFocused('password')} onBlur={() => setFocused(null)} placeholder="••••••••" style={inputStyle('password')} />
            ...
            <button type="submit" disabled={loading} style={{ marginTop: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '15px 0', borderRadius: 12, border: 'none', background: loading ? '#e5e7eb' : 'linear-gradient(135deg,#6c63ff,#8b5cf6)', color: loading ? '#9ca3af' : '#fff', fontSize: 15, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 8px 28px rgba(108,99,255,.45)', transition: 'all .2s', width: '100%' }}>
              {loading ? <><div style={{ width: 16, height: 16, border: '2.5px solid #d1d5db', borderTopColor: '#9ca3af', borderRadius: '50%', animation: 'spin .7s linear infinite' }} />Signing in…</> : <>Sign in <ArrowRight size={16} /></>}
            </button>
          </form>
          ...
          <Link to="/register" style={{ textDecoration: 'none' }}>
            <button style={{ width: '100%', padding: '14px 0', borderRadius: 12, border: '1.5px solid #eaecf0', background: '#fafbff', color: '#374151', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, transition: 'all .2s' }}
              ...
              <UserPlus size={16} color="#6c63ff" /> Create a free account
            </button>
          </Link>
